...
@Component({
  selector: 'app-add-cambio-sim',
  templateUrl: './add-cambio-sim.component.html',
  styleUrls: ['./add-cambio-sim.component.scss']
})
export class AddCambioSimComponent implements OnInit {
  ...
  submit() {
    ...
    for (let index = 0; index < this.validacionPos.item.sim.length; index++) {
      ...
      items.push({
        ...
        sim_id: this.buy.get('operadora').value,
        monto: "150",
      })
    }
    ...
    const data = this.crypto.encryptString(JSON.stringify({
      ...
      sim_serial: this.crypto.encryptJson(this.solicitud.get('serial').value),
      modelo: this.crypto.encryptJson(this.buy.get('operadora').value),
    ...
    this.venta.doCambioDeSim(`${this.session.getDeviceId()};${data}`).subscribe(res => {
  ...
  buscarSim() {
    ...
    this.venta.informacionDeSim(`${this.session.getDeviceId()};${data}`).subscribe(res => {
      console.log(JSON.parse(this.crypto.decryptString(res)))
