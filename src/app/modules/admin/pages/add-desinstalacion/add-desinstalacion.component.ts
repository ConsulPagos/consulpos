...
@Component({
  selector: 'app-add-desinstalacion',
  templateUrl: './add-desinstalacion.component.html',
  styleUrls: ['./add-desinstalacion.component.scss']
})
export class AddDesinstalacionComponent implements OnInit {
 ...
 submit() {
   ...
   const data = this.crypto.encryptString(JSON.stringify({
     ...
     documentos: this.crypto.encryptJson(JSON.stringify([
      this.document.get('CD').value,
    ]))

   }))
   console.log("verify")
   console.log(solicitudes_banco_sell)
   this.venta.crearDesinstalacionDeEquipo(`${this.session.getDeviceId()};${data}`).subscribe(res => {
