...
export class InventarioComponent implements AfterViewInit, OnInit {
  ...
  displayedColumns: string[] = ['id', 'producto', 'stock', 'stock_real', 'action'];
  ...
  ngOnInit(){
    this.loading = true;
    this.title.setTitle('Grupo Altius | Inventario')
   }
    ...
    merge(this.paginator.page)
      .pipe(
        ...
        switchMap(() => {
          this.error = false;
          this.loading = true;
          return this.api.products(this.paginator.pageIndex + 1)
        }),
        map(data => {
